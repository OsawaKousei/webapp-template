...
### 2.3 POST /api/ai/generate-outline
...
### 2.4 POST /api/ai/generate-report

- 目的: overview から本文を生成し、保存済みレポートとして返却
- リクエスト:
...
1. そのまま new-report 入場

### 3.3 /new-report で outline 生成

1. `POST /api/ai/generate-outline`
2. 200 レスポンスの `title` と `outline.items` を画面に反映

### 3.4 /new-report で本文生成 -> /report/:reportId 遷移

1. `POST /api/ai/generate-report` を overview + tone で実行
...
- 404: report 不在時は再生成または一覧戻り導線を表示
